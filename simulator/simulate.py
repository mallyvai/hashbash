...
class Program:
	...
	def _iter_input(self, forward):
		...
		self.memory[c.WI_NUM_BLOCKS] = c.NUM_INPUT_BLOCKS #Index for number of blocks a call to iterinput will create
	...
	def next_step(self):
		...
		if xor_x is c.IMM: x = f_x
		else: x = m[f_x]
		
		if xor_y is c.IMM: y = f_y
		else: y = m[f_y]
		
		if xor_z is c.IMM: z = f_z
		else: z = m[f_z]
		
		if xor_dest is c.IMM: dest = f_dest
		else: dest = m[f_dest]
		
		"""Now lets go and actually do the calculations..."""
		if f_op is c.ops["and"]: m[dest] = x & y
		elif f_op is c.ops["nand"]: m[dest] = ~(x & y)
		elif f_op is c.ops["xor"]: m[dest] = x ^ y
		elif f_op is c.ops["or"]: m[dest] = x | y
		elif f_op is c.ops["not"]: m[dest] = ~x
		
		elif f_op is c.ops["mod"]: m[dest] = x % y
		elif f_op is c.ops["add"]: 
		...
		elif f_op is c.ops["iterinput"]:
			self._iter_input(xor_x == 1)
		
		elif f_op is c.ops["halt"]:
			self.halted = True
...
def prettify_instr(instr):
	...
	return (d(f_dest), d(f_z), d(f_y), d(f_x), d(f_op))
...
def simulate():
	...
	i = 0
	while i < c.MAX_CYCLES and not p.halted:
		#if i % 500 == 0:
		#	print "Cycle", i
		p.next_step()
